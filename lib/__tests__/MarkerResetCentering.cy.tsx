...
// The opening focus zoom must magnify the image by a CONSISTENT factor relative to the fit scale,
// whatever the image/container size. It's applied as a delta on top of the fit scale, and the fit
// scale is inversely proportional to image size — so a fixed additive delta (the old bug) magnifies
// a small-fit-scale view (large image / small container) far more than the intended 1.5x. We pin the
// ratio at a deliberately small viewport, where the fit scale is far from 1 and the additive bug
// would blow the ratio well past 1.5.
describe('the fresh-load focus zoom magnifies by a fixed ratio to the fit scale, not a fixed delta', () => {
  const EXPECTED_ZOOM_RATIO = 1.5; // DEFAULT_ZOOM_FACTOR

  beforeEach(() => {
    // Small viewport => the image fits at a small scale, so the additive-vs-proportional difference
    // is large. At the default viewport the fit scale is ~1, which masks the bug.
    cy.viewport(640, 480);
    localStorage.clear();
  });

  it('opens at ~1.5x the fit-scale canvas width regardless of image size', () => {
    // Fit-scale canvas width: no marker/polygon, so the view stays at fit.
    let fitWidth = 0;
    cy.mount(<Harness />);
    cy.get('canvas').should('exist');
    cy.wait(700);
    container().then($c => (fitWidth = canvasWidthOf($c)));

    // Same viewport, now with a focus target: the view opens auto-zoomed. Its canvas width over the
    // fit width is exactly the zoom factor, since canvas width scales linearly with the total scale.
    cy.then(() => localStorage.clear());
    cy.mount(<Harness marker={{ x: 512, y: 512 }} />);
    cy.get('canvas').should('exist');
    cy.wait(1200);
    container().then($c => {
      const ratio = canvasWidthOf($c) / fitWidth;
      expect(ratio, 'opening magnification is a fixed ratio to fit, not an image-size-dependent delta').to.be.closeTo(EXPECTED_ZOOM_RATIO, 0.15);
    });
  });
});
