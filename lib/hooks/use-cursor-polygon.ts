...
export const useCursorPolygon = () => {
  ...
  const { setPolygons, getNewPolygonColor, isDrawing, polygons, polygon, allowAnnotation, pointRadius, polygonLineColor } = usePolygonContext();
  ...
  useEffect(() => {
    if (cursorCanvasRef.current && polygonCanvasRef.current) {
      ...
      const canvasCursorHandler = new CanvasHandler(cursorCanvas, scaleHandler, pointRadius, polygonLineColor);
      const canvasPolygonHandler = new CanvasHandler(polygonCanvas, scaleHandler, pointRadius, polygonLineColor);
